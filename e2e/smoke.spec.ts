...
test('renders the complete guide and primary landmarks', async ({ page }) => {
  ...
  await expect(heading).toContainText('獨立記者的數位足跡')
  ...
  await expect(page.getByRole('heading', { name: 'Matters 目前提供的隱私與備份措施' })).toBeVisible()
  await expect(page.locator('main')).toContainText('換了名字，其他身分線索還在')
  await expect(page.locator('.trace-story')).toHaveCSS('background-color', 'rgb(245, 243, 255)')
  await expect(page.locator('.trace-story h2')).toHaveCSS('color', 'rgb(0, 0, 0)')
  await expect(page.locator('.platform-requests h2')).toHaveCSS('color', 'rgb(0, 0, 0)')
